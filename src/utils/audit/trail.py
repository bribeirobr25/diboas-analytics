...
from src.utils.audit.checksums import create_data_checksum
...
class AuditTrail:
    ...
    def log_checkpoint(self, name: str, details: Dict[str, Any] = None):
        """
        Log a checkpoint in the execution flow.

        Checkpoints mark significant points in processing for audit trails
        and debugging purposes.

        Args:
            name: Checkpoint name (e.g., 'battle_test_init', 'data_load_complete')
            details: Additional checkpoint details
        """
        self.log_event(
            'checkpoint',
            f"Checkpoint: {name}",
            {'checkpoint_name': name, **(details or {})},
            severity='info'
        )
